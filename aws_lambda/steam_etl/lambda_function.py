...
def lambda_handler(event, context):
    ...
    try:
        ...
        df['id'] = df['id'].astype(str).str.replace('.0', '', regex=False) # Quitar decimales si se leyó como float
        ...
        with table.batch_writer(
            ...
            for _, row in df.iterrows():
                try:
                    ...
                    logger.error(f"Error procesando juego {row.get('id', 'unknown')}: {str(e)}")
                    error_count += 1
                    continue
        ...
        response = {
            ...
            'body': json.dumps({
                ...
                'metrics': {
                    ...
                    'success_rate': f"{(success_count/original_count)*100:.2f}%" if original_count > 0 else "0%"
                }
            })
        }
